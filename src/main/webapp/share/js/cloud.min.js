...
function drawCloud(width, height, divId){

	var max_cloud_size = 6;
	function parseText(t){
		tags=[];
		var e={};
		var maxVal = 0;
		t.split(";split;").forEach(function(t, i){
	...
	function generate(){
		...
		fontSize=d3.scale['log']().range([13, 40]);//log, sqrt, linear 
	...
	function draw(t,e){
		...
		words=t;
		var n=vis.selectAll("text").data(words,function(t){return t.text.toLowerCase()});n.transition().duration(1e3).attr("transform",function(t){return"translate("+[t.x,t.y]+")rotate("+t.rotate+")"}).style("font-size",function(t){return ((t.size < 5) ? 5 : t.size)+"px"}),
		n.enter().append("text").attr("text-anchor","middle").attr("transform",function(t){return"translate("+[t.x,t.y]+")rotate("+t.rotate+")"}).style("font-size","5px").transition().duration(1e3).style("font-size",function(t){return ((t.size < 5) ? 5 : t.size)+"px"}),
	...
	layout=d3.layout.cloud().timeInterval(10).size([w,h]).fontSize(function(t){ return fontSize(+t.value) > 0 ? fontSize(+t.value) : 13;}).text(function(t){return t.key}).on("end",draw),
