...
$(document).ready(function(){
	...
	dhxLayout.cells("b").attachHTMLString('<div id="authchghstInfo" style="width:100%;height:100%;overflow:auto;"></div>');
	setGridHeight();
	myGrid = new dhtmlXGridObject('gridbox');
	myGrid.attachEvent("onRowSelect", load_overview);
	myGrid.setImagePath("${dhtmlXImagePath}");
	myGrid.setHeader("번호,대상자,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,#cspan,수정자,#cspan,#cspan,#cspan",null,grid_head_center_bold);
	myGrid.attachHeader("#rspan,사번,성명,권한,업무,상태,권한부여일자,권한해제일자,비고,사번,성명,권한,접속IP",grid_head_center_bold);
	myGrid.setColumnIds("No,trgterId,trgterKorNm,trgterAuthorCd,trgterAuthorWorkNm,changeSe,authorAlwncDate,authorFrftrDate,mngrResnCn,modUserId,modUserNm,modUserAuthorCd,conectIp");
	myGrid.setInitWidths("50,*,*,*,*,80,*,*,*,*,*,*,*");
	...
	myGrid.setColSorting("na,str,na,str,na,str,str,str,str,str,na,str,str");
...
function getGridRequestURL(){
	var url = "${contextPath}/indvdlinfo/findAuthchghstList.do";
...
function load_overview(rowID,celInd){
	$.post('${contextPath}/indvdlinfo/authchghstForm.do',{'seqNo' : rowID},null,'text').done(function(data){
		$('#authchghstInfo').html(data);
	...
	<!-- START 테이블 1 -->
		...
		<table class="view_tbl mgb_10" >
			...
			<tr>
				<th>대상자사번</th>
				<td>
					<input type="text" id="trgterId" name="trgterId" class="input2" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/>
				</td>
				<th>대상자권한</th>
				<td>
					<select name="trgterAuthorCd" class="select_type" style="width:50%;" onchange="javascript:myGrid_load();">
						<option value="">전체</option>
						<option value="M">관리자</option>
						<option value="D">학과관리자</option>
						<option value="C">단과대관리자</option>
						<option value="T">트랙관리자</option>
						<option value="V">성과열람자</option>
			...
			<tr>
				...
				<th>권한상실여부</th>
				<td>
					<select name="changeSe" class="select_type" style="width:50%;" onchange="javascript:myGrid_load();">
						<option value="">전체</option>
						<option value="Y">유효</option>
						<option value="D">상실</option>
					</select>
				</td>
			</tr>
			<tr>
