...
$(document).ready(function(){
	...
	var header = "번호,관리번호,재산권명,국가,지식재산권명,출원번호,출원일자,등록번호,등록일자,KRI검증,승인여부,최종수정일,P-CODE,분류,Source,삭제여부";
	var columnIds = "No,patentId,itlPprRgtDvsCd,applRegNtnNm,itlPprRgtNm,applRegNo,applRegDate,itlPprRgtRegNo,itlPprRgtRegDate,vrfcDvsCd,apprDvsCd,modDate,familyCode,applDvsCd,srcId,delDvsCd";
	var initWidths = "50,50,50,100,*,100,100,100,100,80,80,80,80,60,80,50";
	var colAlign = "center,center,center,center,left,center,center,center,center,center,center,center,left,center,center,center";
	var colTypes = "ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro";
	var colSorting = "na,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str";

	if(sessMode == 'true' || loginAuthor == 'R' || loginAuthor == 'S')
	{
		header = "<spring:message code='grid.no'/>,<spring:message code='grid.id'/>,<spring:message code='grid.pat2'/>,<spring:message code='grid.pat3'/>,<spring:message code='grid.pat4'/>,<spring:message code='grid.pat8'/>,<spring:message code='grid.pat6'/>,<spring:message code='grid.pat9'/>,<spring:message code='grid.pat7'/>,<spring:message code='grid.kri'/>,<spring:message code='grid.pat.reprsnt'/>,<spring:message code='grid.appr.dvs'/>,P-CODE,분류,<spring:message code='grid.source'/>";
		columnIds = "No,patentId,itlPprRgtDvsCd,applRegNtnNm,itlPprRgtNm,applRegNo,applRegDate,itlPprRgtRegNo,itlPprRgtRegDate,vrfcDvsCd,apprDvsCd,familyCode,isReprsntPatent,applDvsCd,srcId";
		initWidths = "50,50,65,65,*,105,75,75,75,75,65,65,100,60,70";
		colAlign = "center,center,center,center,left,left,center,center,center,center,center,center,left,center,center";
		colTypes = "ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro";
		colSorting = "na,na,str,str,str,str,str,str,str,str,str,str,str,str,str";
	}
...
function getGridRequestURL(){
	...
	var url = "${contextPath}/${preUrl}/patent/findPatentList.do";
...
function myGrid_onRowSelect(rowID,celInd){
	...
	mappingPopup = window.open('about:blank', 'patentPopup', 'width='+wWidth+'px,height='+wHeight+'px,top='+topPos+',left='+leftPos+' location=no,scrollbars=yes,resizable=yes');
	var popFrm = $('#popFrm');
	popFrm.empty();
	popFrm.append($('<input type="hidden" name="patentId" value="'+rowID+'"/>'));
	popFrm.attr('action', '${contextPath}/${preUrl}/patent/patentPopup.do');
	popFrm.attr('target', 'patentPopup');
...
function fn_new(){
	...
	var leftPos = (screenWidth - wWidth)/2;
	var topPos = (screenHeight - wHeight)/2;
	mappingPopup = window.open('about:blank', 'patentNewPopup', 'width='+wWidth+'px,height='+wHeight+'px,top='+topPos+',left='+leftPos+' location=no,scrollbars=yes,resizable=yes');
	var popFrm = $('#popFrm');
	popFrm.empty();
	popFrm.attr('action', '${contextPath}/${preUrl}/patent/patentPopup.do');
	popFrm.attr('target', 'patentNewPopup');
	popFrm.attr('method', 'POST');
	popFrm.submit();
	mappingPopup.focus();
}
function fn_export(){
	var url = "${contextPath}/article/export.do?"+$('#formArea').serialize()+'&'+$('#articleExportFrm').serialize() ;
...
function fn_export(){
	$('#patentExportDialog #closeBtn').triggerHandler('click');
	var url = "${contextPath}/${preUrl}/patent/export.do?"+$('#formArea').serialize()+'&'+$('#patentExportFrm').serialize() ;
	var expAnchor = $('<a class="exp_anchor" href="'+url+'"></a>');
	$("body").append(expAnchor);
	$('a.exp_anchor').bind('click',function(){
		doBeforeGridLoad();
...
<body>
	<div class="title_box">
		<h3><spring:message code='menu.patent'/></h3>
	</div>
	<div class="contents_box">
		...
		<form id="formArea" >
			...
			<table class="view_tbl mgb_10" >
				...
				<tr>
					<th>관리번호</th>
					<td><input type="text" name="patentId" id="patentId" class="input_type" maxlength="10" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/></td>
					<th>삭제구분</th>
					<td>
				...
				<tr>
					<th>출원번호</th>
					<td><input type="text" name="applRegNo" id="applRegNo" class="input_type" maxlength="17" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/></td>
					<th>P-CODE</th>
					<td><input type="text" name="familyCode" id="familyCode" class="input_type" maxlength="15" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/></td>
				</tr>
				<tr>
					<th >소스구분</th>
					<td>
						<input type="radio" id="src_id0" name="srcId" class="radio" value="" checked="checked" onchange="javascript:myGrid_load();"/>
						<label for="src_id0" class="radio_label">전체</label>
						<input type="radio" id="src_id1" name="srcId" class="radio" value="1" onchange="javascript:myGrid_load();"/>
						<label for="src_id1" class="radio_label">PPMS 데이터</label>
						<input type="radio" id="src_id2" name="srcId" class="radio" value="2" onchange="javascript:myGrid_load();"/>
						<label for="src_id2" class="radio_label">직접 입력 데이터</label>
					</td>
					<th>실적 구분</th>
					<td>
						<input type="radio" id="status_all" name="status" class="radio" value="" checked="checked" onchange="javascript:myGrid_load();"/>
						<label for="status_all" class="radio_label">전체</label>
						<input type="radio" id="status_1" name="status" class="radio" value="1" onchange="javascript:myGrid_load();"/>
						<label for="status_1" class="radio_label">거절</label>
						<input type="radio" id="status_2" name="status" class="radio" value="2" onchange="javascript:myGrid_load();"/>
						<label for="status_2" class="radio_label">포기/취하</label>
						<input type="radio" id="status_3" name="status" class="radio" value="3" onchange="javascript:myGrid_load();"/>
						<label for="status_3" class="radio_label">소멸</label>
						<input type="radio" id="status_4" name="status" class="radio" value="4" onchange="javascript:myGrid_load();"/>
						<label for="status_4" class="radio_label">이관</label>
						<input type="radio" id="status_5" name="status" class="radio" value="5" onchange="javascript:myGrid_load();"/>
						<label for="status_5" class="radio_label">분쟁</label>
					</td>
				</tr>
				<tr>
					<th>KRI 검증상태</th>
				...
				<tr>
					<th>지식재산권명</th>
					<td colspan="3">
						<input type="text" id="itlPprRgtNm" name="itlPprRgtNm" class="input_type" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/>
					</td>
				</tr>
				</tbody>
			</table>
			...
			<table class="view_tbl mgb_10" >
				...
				<tbody>
				<tr>
					<th><spring:message code="search.pat1" /></th>
					<td>
						<input type="text" id="itlPprRgtNm" name="itlPprRgtNm" class="input_type" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/>
					</td>
					<th><spring:message code="search.pat2" /></th>
					<td>
						<input type="text" id="stt_date" name="sttDate" class="input_type"  maxlength="4" style="width: 80px;"  onKeyup="javascript:if(event.keyCode=='13')myGrid_load();" />
						~ <input type="text" id="end_date" name="endDate" class="input_type" maxlength="4" style="width: 80px;" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/>
					</td>
		...
		<div class="list_bt_area">
			<div class="list_set">
				<ul>
					...
					<c:if test="${sessMode or (sessionScope.auth.adminDvsCd eq 'R' or  sessionScope.auth.adminDvsCd eq 'S' )}">
						<li><a href="#patentExportDialog" class="modalLink list_icon26">Export</a></li>
					</c:if>
					<li><a href="<c:url value='/${preUrl}/statistics/patent/patent.do'/>"class="list_icon14"><spring:message code='common.button.stats'/></a></li>
				</ul>
			</div>
		</div>
		<!-- layout (grid,paging)  -->
...
<c:if test="${sessMode or (sessionScope.auth.adminDvsCd eq 'R' or  sessionScope.auth.adminDvsCd eq 'S' )}">
	<div id="patentExportDialog" class="popup_box modal modal_layer" style="width: 450px;height:170px; display: none;">
	<form id="patentExportFrm">
		...
		<div class="popup_inner">
			...
			<div class="list_set">
				<ul>
					...
					<li><a href="javascript:$('#patentExportDialog #closeBtn').triggerHandler('click');" class="list_icon10">Cancel</a></li>
				</ul>
			</div>
		</div>
	</form>
