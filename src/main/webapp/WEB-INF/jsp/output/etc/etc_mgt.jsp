...
$(document).ready(function(){
    ...
    var header = "번호,관리번호,사번,성명(한글),실적년월,실적명";
    var columnIds = "no,etcId,userId,korNm,year,subject";
    var initWidths = "40,80,100,110,100,*";
    var colAlign = "center,center,center,center,center,left";
    var colTypes = "ro,ro,ro,ro,ro,ro";
    var colSorting = "na,str,str,str,str,str";

	if(sessMode == 'true' || loginAuthor == 'R' || loginAuthor == 'S')
	{
	    header = "<spring:message code='grid.no'/>,<spring:message code='grid.id'/>,<spring:message code='grid.etc1'/>,<spring:message code='grid.etc2'/>";
	    columnIds = "no,etcId,year,subject";
	    initWidths = "40,80,80,*";
	    colAlign = "center,center,center,left";
	    colTypes = "ro,ro,ro,ro";
	    colSorting = "na,na,str,str";
	}
...
function getGridRequestURL(){
	...
	var url = "${contextPath}/${preUrl}/etc/findAdminEtcList.do";
...
function myGrid_onRowSelect(rowID,celInd) {
	...
	if(rowID) {
		...
		mappingPopup = window.open('about:blank', 'etcPopup', 'width='+wWidth+'px,height='+wHeight+'px,top='+topPos+',left='+leftPos+' location=no,scrollbars=yes,resizable=yes');
		var popFrm = $('#popFrm');
		popFrm.empty();
		popFrm.append($('<input type="hidden" name="etcId" value="'+str[1]+'"/>'));
		popFrm.attr('action', '${contextPath}/${preUrl}/etc/etcPopup.do');
		popFrm.attr('target', 'etcPopup');
...
function fn_export(){
    $('#etcExportDialog #closeBtn').triggerHandler('click');
    var url = "${contextPath}/${preUrl}/etc/export.do?"+$('#formArea').serialize()+'&'+$('#etcExportFrm').serialize() ;
	...
	<div class="contents_box">
		...
		<form id="formArea" >
			...
			<table class="view_tbl mgb_10" >
				...
				<tbody>
					...
					<tr>
						<th>실적명</th>
						<td colspan="3">
							<input type="text" id="subject" name="subject" class="typeText" style="width: 100%;" class="input2" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/>
						</td>
					</tr>
				</tbody>
			</table>
			...
			<table class="view_tbl mgb_10" >
				...
				<tbody>
				<tr>
					<th><spring:message code="search.etc1" /></th>
					<td>
						<input type="text" id="subject" name="subject" class="typeText" style="width: 100%;" class="input2" onKeyup="javascript:if(event.keyCode=='13')myGrid_load();"/>
					</td>
					<th><spring:message code="search.etc2" /></th>
					<td>
		...
		<div class="list_bt_area">
			<div class="list_set">
				<ul>
					<c:if test="${sessionScope.auth.ETC gt 1 and (sessMode or (sessionScope.auth.adminDvsCd eq 'R' or  sessionScope.auth.adminDvsCd eq 'S' )) }">
						<li><a href="#" onclick="javascript:myGrid_onRowSelect();" class="list_icon12"><spring:message code='common.button.new'/></a></li>
					</c:if>
                    <c:if test="${sessMode or (sessionScope.auth.adminDvsCd eq 'R' or  sessionScope.auth.adminDvsCd eq 'S' )}">
                        <li><a href="#etcExportDialog" class="modalLink list_icon26">Export</a></li>
                    </c:if>
	                <c:if test="${not sessMode and sessionScope.auth.adminDvsCd ne 'R' and  sessionScope.auth.adminDvsCd ne 'S'}">
...
<c:if test="${sessMode or (sessionScope.auth.adminDvsCd eq 'R' or  sessionScope.auth.adminDvsCd eq 'S' )}">
       <div id="etcExportDialog" class="popup_box modal modal_layer" style="width: 450px;height:170px; display: none;">
       <form id="etcExportFrm">
              <div class="popup_header">
                     <h3>Other Research Activities Export</h3>
              ...
              <div class="popup_inner">
                     ...
                     <div class="list_set">
                           <ul>
                                  ...
                                  <li><a href="javascript:$('#etcExportDialog #closeBtn').triggerHandler('click');" class="list_icon10">Cancel</a></li>
                           </ul>
                     </div>
              </div>
       </form>
