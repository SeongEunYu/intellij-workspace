...
<head>
    <script type="text/javascript">
        ...
        function goDocument(page){
            ...
            $(location).attr('href',"${pageContext.request.contextPath}/personal/myRss/nBoard.do?"+addr);
        }
...
<div class="sub_container">
    ...
    <h3 class="h3_title">Notice Board</h3>
                        ...
                        <c:choose>
                            <c:when test="${fn:length(boardList) > 0}">
                                <c:forEach items="${boardList}" var="board">
                                    <tr>
                                        <td class='al_left'><a href="${pageContext.request.contextPath}/personal/myRss/nBoardDetail.do?bbsId=${board.bbsId}&page=${page}&order=${order}&sort=${sort}">${board.title}</a></td>
                                        <td class='al_center' style="padding-left: 0px; padding-right: 0px"><span style="font-size: 13px">${board.regUserId}</span></td>
                                        <td class='al_center' style="padding-left: 0px; padding-right: 0px"><span style="font-size: 13px">${board.viewCnt}</span></td>
                                        <td class='al_center' style="padding-left: 0px; padding-right: 0px"><span style="font-size: 13px"><fmt:formatDate value="${board.regDate}" pattern="yyyy-MM-dd"/></span></td>
                                    </tr>
                                </c:forEach>
                            </c:when>
                            <c:otherwise>
