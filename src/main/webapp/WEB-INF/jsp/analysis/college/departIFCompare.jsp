...
<head>
<title><spring:message code="menu.asrms.clg.avgif"/></title>
...
.to_inner span {min-width: 70px;}
 ...
 function departIFAjax(idx){
	 ...
	 $.ajax({
		 url:"<c:url value="/analysis/college/departIFAjax.do"/>",
		 dataType: "json",
		 data: $('#frm').serialize(),
		 method: "POST",
...
//Callback handler method which is invoked after the chart has saved image on server.
 ...
 function saveExcel(fileName){
	...
	div.append(table);
	 ...
	 var excelFileName = "DepartmentImpactFactorCompare_"+$('#gubun').val() + "_from_" + $('#fromYear').val() +"_to_" + $('#toYear').val() + ".xls";
...
<body>
<h3 class="page_title"><spring:message code="menu.asrms.clg.avgif"/></h3>
<div class="help_text mgb_30"><spring:message code="asrms.college.impactfactor.comp.desc"/></div>

<form id="frm" name="frm" action="${contextPath}/analysis/college/departIF.do" method="post">
