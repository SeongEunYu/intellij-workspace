...
<body>
	<form id="frm" name="frm" action="${contextPath}/analysis/college/topArticles.do" method="post">
	...
	<div class="sub_content_wrapper" style="width: 735px;overflow: auto;">
		<table width="800px;" id="publicationsTbl" class="list_tbl mgb_20">
			<colgroup>
				<col style="width: 5%"/>
				<col style="width: 50%"/>
				<col style="width: 7%"/>
				...
				<c:if test="${fn:length(artList) > 0}">
					<c:forEach items="${artList}" var="item" varStatus="status">
						...
						<td style='font-size: 10pt;' align="center" width="40">
							${status.count}</td>
						<td align="left" style='font: normal 12px 'Malgun Gothic';'>
							<div class="style_12pt"><a href="javascript:popArticle(${item.SEQNO})"><b>${item.ESUBJECT}</b></a></div>
						</td>
						<td style="text-align: right;padding-right: 5px;"><div>${item.TC}</div></td>
						<td style="padding-left: 15px; ">
							<div>
								<ul>
								<c:set var="author" value="${fn:split(item.AUTHORS, ';')}"/>
								<c:forEach var="ai" items="${author}" varStatus="idx">
									<c:set var="name" value=""/>
									<c:set var="userid" value=""/>
									<c:set var="grade" value=""/>
									<c:set var="depart" value=""/>
									<c:set var="info" value="${fn:split(ai, ',')}"/>
									<c:forEach var="i" items="${info}" varStatus="a">
										<c:if test="${a.count eq 1 }"><c:set var="name" value="${i}"/></c:if>
										<c:if test="${a.count eq 2 }"><c:set var="userid" value="${i}"/></c:if>
										<c:if test="${a.count eq 3 }"><c:set var="grade" value="${i}"/></c:if>
										<c:if test="${a.count eq 4 }"><c:set var="depart" value="${i}"/></c:if>
									</c:forEach>
										<li>${name }(${userid })&nbsp;/&nbsp;${grade }&nbsp; <c:if test="${not empty depart }">/&nbsp; ${depart }</c:if></li>
								</c:forEach>
								</ul>
							</div>
						</td>
      ...
      <div style="display: none;" id="excelExportDiv">
		...
		<c:if test="${fn:length(artList) > 0}">
			<c:forEach items="${artList}" var="item" varStatus="status">
				...
				<td align="right" style='font: normal 12px 'Malgun Gothic';'>${item.KCI_TC}</td>
