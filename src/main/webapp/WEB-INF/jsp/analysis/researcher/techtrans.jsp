...
<head>
<title><spring:message code="menu.asrms.rsch.techtrans"/></title>
...
function tcntrnsAjax(idx){
    ...
    $.ajax({
        url:"<c:url value="/analysis/researcher/tcntrnsAjax.do"/>",
        dataType: "json",
        data: $('#frm').serialize(),
        method: "POST",
...
//Callback handler method which is invoked after the chart has saved image on server.
 ...
 function saveExcel(fileName){
	 ...
	 var dataTbl = $('<tr><td><h1>Data</h1></td></tr><tr><td>'+$('#publicationsTbl').clone().wrapAll('<div/>').parent().html()+'</td></tr>');
	 table.append(pageTitle)
     .append(chartTr)
     .append(dataTbl);
	 div.append(table);
	 $('#tableHTML').val(div.html());
	 var excelFileName = "TransferOfTechnology_from_" + $('#fromYear').val() +"_to_" + $('#toYear').val() + ".xls";
...
<body>
	<h3 class="page_title"><spring:message code="menu.asrms.rsch.techtrans"/></h3>
	<div class="help_text mgb_30"><spring:message code="asrms.researcher.techtrans.desc"/></div>
		...
		<div class="to_inner">
			...
			<em>
				<select name="techTransrCd" id="techTransrCd">
					...
					<option value="1">Sub-license</option>
					<option value="2">Exclusive License</option>
					<option value="3">Non-Exclusive License</option>
					<option value="4">Know-how</option>
					<option value="5">Counsel</option>
	...
	<table width="100%" id="publicationsTbl" class="list_tbl mgb_20">
		...
		<tr>
			...
			<th><span>Sub-license</span></th>
			<th><span>Exclusive License</span></th>
			<th><span>Non-Exclusive License</span></th>
			<th><span>Know-how</span></th>
