...
<head>
	...
	<script type="text/javascript">
        ...
        var tab_activate = function(event, ui){
            ...
            {
                var tcChartObject = getChartFromId('tcChart');
                if(tcChartObject == null || !tcChartObject.hasRendered()) drawCiationChart();
            }
        ...
        var drawCiationChart = function(){
            var tcChartOpt = $.extend(true, {}, chartOpt);
            tcChartOpt['id'] = 'tcChart';
            tcChartOpt['type'] = 'mscombidy2d';
            tcChartOpt['renderAt'] = 'ciation_tab_chart_container';
            tcChartOpt['height'] = '350';
            tcChartOpt.dataSource.chart['exportHandler'] = '${contextPath}/servlet/FCExporter/export.do';
            tcChartOpt.dataSource.chart['exportCallBack'] = 'export_tc_chart';

            $.post("${contextPath}/analysis/department/difUnivComapreTcChartData.do", $('#frm').serializeArray(),null,'json').done(function(data){
                $('#ciation_tab_chart_container').off('fusionchartsrendercomplete').on('fusionchartsrendercomplete',function(eventObj, dataObj){});
                tcChartOpt.dataSource['categories'] = data.categories;
                tcChartOpt.dataSource['dataset'] = data.dataset;
                $('#ciation_tab_chart_container').insertFusionCharts(tcChartOpt);
            });
        }
        ...
        var detoryChart = function(){
            if(FusionCharts('tcChart')) FusionCharts('tcChart').dispose();
            if(FusionCharts('artChart')) FusionCharts('artChart').dispose();
            $('#ciation_tab_chart_container').disposeFusionCharts().empty();
        ...
        function exportExcel(){
            ...
            setTimeout(function() {
                var tcChartObj = getChartFromId('tcChart');
                if( tcChartObj.hasRendered() ) tcChartObj.exportChart( { exportFormat : 'png'} );
            }, 1000);
        }

        function export_tc_chart(objRtn){
            ...
            {
                dhtmlx.alert({type:"alert-warning",text:"The TC chart could not be saved on server. There was an error. Description : " + objRtn.statusMessage,callback:function(){}});
            }
        }
...
<h3 class="page_title">미국대학 vs KAIST Citation 비교</h3>
<div class="help_text mgb_30"><spring:message code="asrms.department.other.univ.comp.citation.desc"/></div>

<form id="frm" name="frm">
	<input type="hidden" name="deptKor" id="deptKor" value="${param.deptKor}"/>
	...
	<div class="help_text mgb_30"><spring:message code="asrms.department.other.univ.comp.citation.chart.desc"/></div>
	<div class="sub_content_wrapper">
		<div id="tabs" class="tab_wrap">
			<ul>
				<li><a href="#tabs-1">Citation</a></li>
				<li><a href="#tabs-2">논문수</a></li>
			</ul>
			<div id="tabs-1">
				<div id="ciation_tab_chart_container" class="chart_box"></div>
			</div>
			<div id="tabs-2">
	...
	<p class="bt_box mgb_20"><a href="javascript:void(0);" onclick="exportExcel();" class="black_bt"><em class="export_icon">Export</em></a></p>
...
<form id="excelFrm" action="${contextPath}/analysis/department/exportExcel.do" method="post">
	...
	<input type="hidden" id="expFileName" name="expFileName" value="difUnivCompareTc.xlsx" />
</form>
</body>
</html>
