...
<script type="text/javascript">
 ...
 function fulltimeSciAjax(){
     ...
     $.ajax({
         url:"<c:url value="/analysis/department/fulltimeSciAjax.do"/>",
         dataType: "json",
         data: $('#frm').serialize(),
         method: "POST",
         ...
         var $tbody = "";
         for(var i=0; i<data.fultimeSciYearList.length; i++){
             var fl = data.fultimeSciYearList[i];
         ...
         if(data.fultimeSciYearList.length == 0){
 ...
 function saveExcel(){
	 ...
	 var excelFileName = "FulltimeSCIArticleAnalysis_"+$('#gubun').val() + "_from_" + $('#fromYear').val() +"_to_" + $('#toYear').val() + '_StandDate_' +$('#stndMonthDay').val() + ".xls";
...
<body>
	...
	<h3 class="page_title"><spring:message code="menu.asrms.dept.fulltimeSci"/></h3>
	<div class="help_text mgb_30"><spring:message code="asrms.department.fulltime.sci.desc"/></div>
		...
		<table width="100%" id="publicationsTbl" class="list_tbl mgb_20">
			...
			<thead>
				<tr style="text-align: center;height:25px">
					...
					<th><span>논문당평균<br/>피인용횟수</span></th>
