...
<title><spring:message code="menu.asrms.inst.fulltimeFunding"/></title>
<script type="text/javascript">
 ...
 function fulltimeFundingAjax(idx){
     ...
     $.ajax({
         url:"<c:url value="/analysis/institution/fulltimeFundingAjax.do"/>",
         dataType: "json",
         data: $('#frm').serialize(),
         method: "POST",
...
//Callback handler method which is invoked after the chart has saved image on server.
 ...
 function saveExcel(fileName){
	 ...
	 var excelFileName = "fulltimeFunding_"+ $('#stndYear').val() + "_"  + $('#stndMonthDay').val() + "_from_" + $('#fromYear').val() +"_to_" + $('#toYear').val() + ".xls";
...
<body>
<h3 class="page_title"><spring:message code="menu.asrms.inst.fulltimeFunding"/></h3>
<div class="help_text mgb_30"><spring:message code="asrms.institution.fulltimeFunding.desc"/></div>
<form id="frm" name="frm" action="${contextPath}/analysis/institution/fulltimeFunding.do" method="post">
