...
<head>
<title><spring:message code="menu.asrms.inst.patentByResearchers"/></title>
...
<script>
 ...
 function patentByResearchersAjax(idx){
     ...
     $.ajax({
         url:"<c:url value="/analysis/institution/patentByResearchersAjax.do"/>",
         dataType: "json",
         data: $('#frm').serialize(),
         method: "POST",
         ...
         if(data.userList.length == 0){
             var  $tr = '<tr style="background-color: #ffffff;" height="17px"><td style="font-size: 10pt;" align="center" colspan=4><img src="${contextPath}/images/layout/ico_info.png" />검색된 결과가 없습니다.</td></tr>';
             $tbody += $tr;
         }
         ...
         if(data.userList.length > 0){
			 if(idx == '0'){
				 $("#patentTbl").tablesorter({ sortList:[[4,1]] });
			 }else{
...
<body>
<h3 class="page_title"><spring:message code="menu.asrms.inst.patentByResearchers"/></h3>
<div class="help_text mgb_30"><spring:message code="asrms.institution.patentByResearchers.desc"/></div>
