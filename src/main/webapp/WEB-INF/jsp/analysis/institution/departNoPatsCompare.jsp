...
<head>
<title><spring:message code="menu.asrms.inst.patent"/></title>
...
<script type="text/javascript">
 ...
 function departNoPatsAjax(){
     ...
     $.ajax({
         url:"<c:url value="/analysis/institution/departNoPatsAjax.do"/>",
         dataType: "json",
         data: $('#frm').serialize(),
         method: "POST",
         ...
         for(var i=0; i<data.departList.length; i++){
             ...
             $tr += '<input type="checkbox"  class="dept_chb" id="no_chkbox_'+dl.deptKor+'" value="'+dl.deptKor+'"';
 ...
 function clickCheckbox(obj){
		...
		var chb = $('.dept_chb:checkbox:checked');
		for(var j=0; j < chb.length; j++){
			var fcIdx = $('.dept_chb:checkbox').index(chb.eq(j)) % 23;
			var deptKor = chb.eq(j).val();
			var color = fillColors[fcIdx-1];
...
<body>
<h3 class="page_title"><spring:message code="menu.asrms.inst.patent"/></h3>
<div class="help_text mgb_30"><spring:message code="asrms.institution.patent.desc"/></div>
