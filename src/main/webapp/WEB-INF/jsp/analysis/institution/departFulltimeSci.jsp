...
<script type="text/javascript">
 ...
 $(function() {
	...
	$("#tabs").css("display", "block");
     ...
     departFulltimeSciAjax();
  });

 function departFulltimeSciAjax(){
     $.ajax({
         url:"<c:url value="/analysis/institution/departFulltimeSciAjax.do"/>",
         dataType: "json",
         data: $('#frm').serialize(),
         method: "POST",
         ...
         if(data.fulltimeSciSelectedYearList.length > 0){
             ...
             for(var j=0; j < data.fulltimeSciSelectedYearList.length; j++){
                 var fl = data.fulltimeSciSelectedYearList[j];

                 var $tr = '<tr style="height:17px;">';
                 $tr += '<td style="padding-left: 5px;">'+fl.deptKor+'</td>';
                 ...
                 $tr += '<td class="center">'+fl.impctFctrSum.toFixed(2).replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,')+'</td>';
                 $tr += '<td class="center">'+fl.citedAvrg.toFixed(2).replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,')+'</td>';
                 $tr += '<td class="center">'+fl.artsAvrg.toFixed(2).replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,')+'</td>';
                 $tr += '<td class="center">'+fl.impctFctrAvrgByRsrch.toFixed(2).replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,')+'</td>';
                 $tr += '</tr>';
 ...
 function exportUserByArticlePopup(dept){
     var userWindow =  window.open("exportUserByArticlePopup.do?"+$("#frm").serialize()+"&searchDept="+encodeURIComponent(dept),"userWindow", "top=100, left=350, height=600, width=800, location=no, resizable=no, menubar=no");
     userWindow.focus();
 }
 ...
 function saveExcel(){
	 ...
	 var excelFileName = "DepartmentFulltimeSci_"+$('#stndMonthDay').val() + "selectedYear_" + $('#selectedYear').val() + ".xls";
...
<body>
	<h3 class="page_title"><spring:message code="menu.asrms.inst.departFulltimeSci"/></h3>
	<div class="help_text mgb_30"><spring:message code="asrms.institution.fulltime.sci.dept.desc"/></div>
	...
	<input type="hidden" name="isStndChanged" id="isStndChanged" value="false"/>
		...
		<div class="top_option_box">
			...
			<p class="ts_bt_box">
				<a href="javascript:departFulltimeSciAjax();" class="to_search_bt"><span>Search</span></a>
			</p>
		</div>
	</form>
