...
<head>
<title><spring:message code="menu.asrms.inst.fundingByGov"/></title>
...
th.header {
 ...
 function fundingByGovAjax(idx){
     if(!validateRange()){errorMsg(this); return false;}

     $.ajax({
         url:"<c:url value="/analysis/institution/fundingByGovAjax.do"/>",
         dataType: "json",
         data: $('#frm').serialize(),
         method: "POST",
         ...
         if(data.fundingDetailList.length > 0){
             ...
             for(var i=0; i<data.fundingDetailList.length; i++){
                 ...
                 $tr += '<input type="checkbox"  id="no_chkbox_'+fd.id+'" value="'+fd.cptGovOfficNm+'"';
                 if(i < 5) $tr += 'checked="checked"';
                 $tr += '/><input type="hidden" name="no_fillColorIndex_'+fd.id+'" id="no_fillColorIndex_'+fd.id+'" value="'+fc+'" /></td>';
                 $tr += '<td class="center">'+fd.cptGovOfficNm+'</td>';
	...
	function printDataTable(){
		...
		var trTh = $('<tr style="height:20px;"><th><span>소관<br/>정부부처</span></th>');
		var countCol = 0;
		for(var i=fy; i <= ty; i++){
		...
		for(var j=0; j < djArr.length; j++){
		 var name = djArr[j].cptGovOfficNm ;
		 ...
		 for(var t=0; t < djArrWithY.length; t++){
			 ...
			 if(name == djArrWithY[t].cptGovOfficNm){
	...
	function clickCheckbox(obj){
		...
		for(var i=0; i < djArr.length; i++){
			var size = djArr[i].cptGovOfficNm.length;
			var id = "";
			var standardY = "";
			var count = 0;
			for(var t=0; t<size; t++){
				id += djArr[i].cptGovOfficNm.charAt(t).charCodeAt(0);
			}
			if($('#no_chkbox_'+id).is(':checked')){
				var color = fillColors[$('#no_fillColorIndex_'+id).val()];
				dataset += "<dataset seriesName='"+djArr[i].cptGovOfficNm+"' color='"+color+"' id='"+id+"' >";
				dataset2 += "<dataset seriesName='"+djArr[i].cptGovOfficNm+"' color='"+color+"' id='"+id+"' >";
				...
				for(var j=0; j < djArrWithY.length; j++){
					...
					if(djArr[i].cptGovOfficNm == djArrWithY[j].cptGovOfficNm){
						var val = djArrWithY[j].rsrcctSum == undefined ? 0 : djArrWithY[j].rsrcctSum;
						var toolText = djArrWithY[j].cptGovOfficNm+" ("+djArrWithY[j].rsrcctContYr+") :" + val.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
						var val2 = djArrWithY[j].totRsrcct == undefined ? 0 : parseInt(Number(djArrWithY[j].totRsrcct)/1000000);
						var toolText2 = djArrWithY[j].cptGovOfficNm+" ("+djArrWithY[j].rsrcctContYr+") :" + val2.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+"(단위:백만원)";
					...
					if(djArrWithY[j].rsrcctContYr != standardY && count != (Number(standardY)-Number(djArrWithY[0].rsrcctContYr))){
						toolText = djArr[i].cptGovOfficNm+" ("+djArrWithY[j].rsrcctContYr+") : 0";
	...
	function saveExcel(){
		...
		var excelFileName = "fundingByGov_from_" + $('#fromYear').val() +"_to_" + $('#toYear').val() + ".xls";
...
<body>
	<h3 class="page_title"><spring:message code="menu.asrms.inst.fundingByGov"/></h3>
	<div class="help_text mgb_30"><spring:message code="asrms.institution.fundingByGov.desc"/></div>
	...
	<div class="top_option_box">
		...
		<p class="ts_bt_box">
			<a href="javascript:fundingByGovAjax();" class="to_search_bt"><span>Search</span></a>
		</p>
	</div>
	...
	<div id="tabs" class="tab_wrap mgb_10" style="display: none;">
	  <ul>
	    ...
	    <li><a href="#tabs-4">Data(금액)</a></li>
  		...
  		<div id="tabs-1">
		   <div id="chartdiv1" class="chart_box" align="left">
		   	</div>
		</div>
		<div id="tabs-2">
