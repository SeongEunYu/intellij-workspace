...
<c:if test="${not empty openFileList}">
	<c:forEach items="${openFileList}" var="ol" varStatus="idx">
		<row id='${ol.conferenceId}'>
			<cell></cell>
			<cell>${idx.count}</cell>
			<cell>${ol.conferenceId}</cell>
			<cell>${fn:escapeXml(ol.orgLangPprNm)}</cell>
			<cell>${fn:escapeXml(ol.scjnlNm)}</cell>
			<cell>${rims:toDateFormatToken(ol.ancmDate, '.')}</cell>
			<cell>
				<c:if test="${empty ol.isOpen}">작업대기</c:if>
				<c:if test="${ol.isOpen eq 'Y'}">원문공개</c:if>
				<c:if test="${ol.isOpen eq 'N'}">원문미공개</c:if>
			</cell>
			<cell>
