...
function loadJcrCellComponent(){
	...
	jcrGrid.setHeader("번호,Year,Journal Name,등재,ISSN,Citation(2년),KCI I.F.", null, grid_head_center_bold);
    jcrGrid.setColumnIds("id,prodyear,title,regstAt,issn,ctsDoc2yrs,kciIf");
	jcrGrid.setInitWidths("50,60,*,80,80,80,60");
	jcrGrid.setColAlign("left,center,left,center,center,center,center,center");
...
function loadHistCellComponent(){
	...
	histGrid.setHeader("Year,Citation(2년),KCI I.F.", null, grid_head_center_bold);
...
function getGridRequestURL(){
	var url = '<c:url value="/${preUrl}/kci/findKciTitlesList.do"/>';
...
function showhistGrid(strJnl,strIssn){
	...
	var url = '<c:url value="/${preUrl}/kci/findHistoryList.do?title="/>'+strJnl+"&issn="+strIssn;
...
function shwosbjtGrid(rowInd){
	...
	var url = '<c:url value="/${preUrl}/kci/findSubjectList.do?id="/>'+rowInd;
...
<body>
	<!-- Page Title -->
	<div class="title_box">
		<h3><spring:message code='menu.kci'/></h3>
	</div>
	<!-- Main Content -->
