...
<script type="text/javascript">
	var swfu, upload_url = "${contextPath}/import/uploadFile.do?gubun=<%=R2Constant.SOURC_TYPE_KCIIF%>";
	...
	$(document).ready(function(){
		...
		myVault = impLayout.cells("b").attachVault({
		    ...
		    uploadUrl:  "${contextPath}/import/vaultUploadFile.do?gubun=WOS&format=<%=R2Constant.SOURC_FILE_FORMAT_XLS %>",           // html4/html5 upload url
		    swfPath:    "dhxvault.swf",         // path to flash uploader
		    swfUrl:     "${contextPath}/import/vaultUploadFile.do?gubun=WOS&format=<%=R2Constant.SOURC_FILE_FORMAT_XLS %>",           // flash upload url
		    slXap:      "dhxvault.xap",         // path to silverlight uploader
		    slUrl:      "${contextPath}/import/vaultUploadFile.do?gubun=WOS&format=<%=R2Constant.SOURC_FILE_FORMAT_XLS %>" // silverlight upload url, FULL path required
		});
		...
		formData = [
					...
					{type: "radio", name: "format", value: "<%=R2Constant.SOURC_FILE_FORMAT_XLS %>", label: "Excel File", checked:true}
				];
		myForm = impLayout.cells("a").attachForm(formData);

		myForm.attachEvent("onChange", function(name,value,is_checked){
			myVault.setURL("${contextPath}/import/vaultUploadFile.do?gubun=<%=R2Constant.SOURC_TYPE_KCIIF%>&format="+value);
			myVault.setSLURL("${contextPath}/import/vaultUploadFile.do?gubun=<%=R2Constant.SOURC_TYPE_KCIIF%>&format="+value);
			myVault.setSWFURL("${contextPath}/import/vaultUploadFile.do?gubun=<%=R2Constant.SOURC_TYPE_KCIIF%>&format="+value);
		});
	...
	function getGridRequestURL(){ return "${contextPath}/import/findRdHisotryList.do?gubun=<%=R2Constant.SOURC_TYPE_KCIIF%>"; }
...
<form id="sForm" name="sForm" method="post" action="${contextPath}/import/parseKciIF.do?utilType=exr">
	<input type="hidden" name="pMenuId" value="${param.pMenuId}"/>
	<!-- Page Title -->
	<div class="title_box">
		<h3><spring:message code="menu.kci_if.import"/></h3>
	</div>
	<!-- Main Content -->
