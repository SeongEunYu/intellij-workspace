...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		window.dhx4.ajax.get("${contextPath}/import/upload_conf.do?gubun=WOS&format=<%=R2Constant.SOURC_FILE_FORMAT_TAB %>",function(r){
		...
		formData = [
					...
					{type: "radio", name: "format", value: "<%=R2Constant.SOURC_FILE_FORMAT_TAB %>", label: "탭으로 구분된 형식(Win, UTF-8)", checked:true},
					{type: "radio", name: "format", value: "<%=R2Constant.SOURC_FILE_FORMAT_PLAIN %>", label: "Plain Text"}
				];
		myForm = impLayout.cells("a").attachForm(formData);

		myForm.attachEvent("onChange", function(name,value,is_checked){
			myVault.setURL("${contextPath}/import/uploadHandler.do?gubun=WOS&format="+value);
			myVault.setSLURL("${contextPath}/import/uploadHandler.do?gubun=WOS&format="+value);
			myVault.setSWFURL("${contextPath}/import/uploadHandler.do?gubun=WOS&format="+value);
		});
...
<form id="sForm" name="sForm" method="post" action="${contextPath}/import/parseWos.do?utilType=exr">
	<input type="hidden" name="pMenuId" value="${param.pMenuId}"/>
	<!-- Page Title -->
	<div class="title_box">
		<h3><spring:message code="menu.wos.import"/></h3>
	</div>
	<!-- Main Content -->
