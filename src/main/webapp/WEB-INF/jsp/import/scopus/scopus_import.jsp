...
$(document).ready(function(){
	...
	window.dhx4.ajax.get("${contextPath}/import/upload_conf.do?gubun=SCP&format=<%=R2Constant.SOURC_FILE_FORMAT_CSV %>",function(r){
		var t = window.dhx4.s2j(r.xmlDoc.responseText);
		if(t != null)
		{
			myVault = impLayout.cells("b").attachVault(t);
	...
	formData = [
				...
				{type: "radio", name: "format", value: "<%=R2Constant.SOURC_FILE_FORMAT_CSV %>", label: "CSV 포맷", checked:true}
			];
	myForm = impLayout.cells("a").attachForm(formData);

	myForm.attachEvent("onChange", function(name,value,is_checked){
		myVault.setURL("${contextPath}/import/uploadHandler.do?gubun=SCP&format="+value);
		myVault.setSLURL("${contextPath}/import/uploadHandler.do?gubun=SCP&format="+value);
		myVault.setSWFURL("${contextPath}/import/uploadHandler.do?gubun=SCP&format="+value);
	});
...
function onFileUploadComplete(file,extra){
	...
	tr.append($('<td>'+extra.count+'</td>').append($('<input type="hidden" name="totcount" value="'+extra.count+'"/>')).append($('<input type="hidden" name="fformat" value="'+extra.format+'"/>')).append($('<input type="hidden" name="furl" value="'+extra.url+'"/>')));
...
function getGridRequestURL(){return "${contextPath}/import/findRdHisotryList.do?gubun=SCP";}
...
<body>
<form id="sForm" name="sForm" method="post" action="${contextPath}/import/parseScopus.do?utilType=exr">
	<input type="hidden" name="pMenuId" value="${param.pMenuId}"/>
	<!-- Page Title -->
	<div class="title_box">
		<h3><spring:message code="menu.scopus.import"/></h3>
	</div>
	<!-- Main Content -->
	<div class="contents_box">
		...
		<script type="text/javascript">$('#mainLayout').css('height',($(document).height()-(520 + $('.header_wrap').height() + $('.nav_wrap').height()))+"px");</script>
